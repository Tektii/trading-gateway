...
use helpers::{binance_futures_trade_json, test_futures_adapter};
...
async fn get_trades_success() {
    ...
    mount_json(
        ...
        "/fapi/v1/userTrades",
        200,
        json!([binance_futures_trade_json("BTCUSDT")]),
    )
    .await;
    ...
    assert_eq!(trades[0].order_id, "456789");
}
...
async fn get_trades_with_symbol() {
    ...
    mount_json(
        ...
        "/fapi/v1/userTrades",
        200,
        json!([binance_futures_trade_json("ETHUSDT")]),
    )
    .await;
...
async fn get_trades_empty() {
    ...
    mount_json(&server, "GET", "/fapi/v1/userTrades", 200, json!([])).await;

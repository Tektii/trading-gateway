...
use helpers::{binance_margin_trade_json, test_margin_adapter};
...
async fn get_trades_success() {
    ...
    mount_json(
        ...
        200,
        json!([binance_margin_trade_json("BTCUSDT")]),
    )
    .await;
    ...
    assert_eq!(trades[0].order_id, "789012");
}
...
async fn get_trades_with_symbol() {
    ...
    mount_json(
        ...
        "/sapi/v1/margin/myTrades",
        200,
        json!([binance_margin_trade_json("ETHUSDT")]),
    )
    .await;
...
async fn get_trades_empty() {
    ...
    mount_json(&server, "GET", "/sapi/v1/margin/myTrades", 200, json!([])).await;

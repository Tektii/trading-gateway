...
use helpers::{binance_spot_kline_json, binance_spot_quote_json, test_margin_adapter};
...
async fn get_quote_error() {
    ...
    // Margin get_quote uses raw reqwest without error mapper, so a 500 response
    // with non-matching JSON structure results in a parse error (Internal).
    mount_json(
        &server,
        "GET",
        "/api/v3/ticker/bookTicker",
...
async fn get_bars_empty() {
    ...
    mount_json(&server, "GET", "/api/v3/klines", 200, json!([])).await;

...
use helpers::{binance_futures_account_json, test_futures_adapter};
...
async fn get_account_success() {
    ...
    mount_json(
        ...
        "/fapi/v2/account",
        200,
        binance_futures_account_json(),
    )
    .await;

    let account = adapter.get_account().await.unwrap();
    // balance = availableBalance, equity = totalMarginBalance
    assert_eq!(account.balance, dec!(90000));

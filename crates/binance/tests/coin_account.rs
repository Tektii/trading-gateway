...
use helpers::{binance_coin_account_json, test_coin_futures_adapter};
...
async fn get_account_success() {
    ...
    mount_json(
        ...
        "/dapi/v1/account",
        200,
        binance_coin_account_json(),
    )
    .await;

    let account = adapter.get_account().await.unwrap();
    assert_eq!(account.balance, dec!(4.5));
    assert_eq!(account.equity, dec!(5.1));
